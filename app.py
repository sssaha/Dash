...
feature_importance = pd.read_csv(total_path + feature_name).sort_values(
    by=["Importance"], ascending=False
)

program_data_folder = os.path.join("/home", "programdata")
...
def filter_dataframe(input_df, var1, var2, var3):
    ...
    input_df = input_df[
        (input_df["high_blood_pressure"].isin(bp_list))
        ...
        [
            dbc.Card(
                dbc.CardBody(
                    [
                        dcc.Graph(
                            figure=input_figure.update_layout(
...
sidebar = html.Div(
    children=[
        ...
        html.Hr(),
        html.P(
            "Tutorial project detailing how to develop a basic front end application exploring the factors influencing heart failure",
            className="lead",
        ),
        html.H3("Model"),
        html.P(
            ...
            [
                html.Div(
                    children=[
                        ...
                        dcc.Markdown(
                            "A comprehensive tool for examining factors impacting heart failure"
                        ),
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": i, "value": i}
                                for i in df1["high_blood_pressure"].drop_duplicates()
                            ]
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": i, "value": i}
                                for i in df1["sex"].drop_duplicates()
                            ]
                            + [{"label": "Select All", "value": "all_values"}],
                            value="all_values",
                        ),
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": i, "value": i}
                                for i in df1["anaemia"].drop_duplicates()
                            ]
                            + [{"label": "Select All", "value": "all_values"}],
                            value="all_values",
                        ),
                        ...
                        html.Div(
                            [
                                html.A(
                                    ...
                                    href="https://www.kaggle.com/datasets/andrewmvd/heart-failure-clinical-data?select=heart_failure_clinical_records_dataset.csv",
                                    target="_blank",
                                )
                            ],
...
def update_output_div(bp, sex, anaemia):
    ...
    return dbc.Row(
        [
            dbc.Col([draw_Image(factor_fig)], width={"size": 3, "offset": 0}),
            dbc.Col([draw_Image(age_fig)], width={"size": 3}),
            dbc.Col([draw_Image(time_fig)], width={"size": 3}),
        ]
    )
...
def update_model(value):
    ...
    confusion_fig = px.imshow(
        confusion,
        labels=dict(x="Predicted Value", y="True Value", color="Prediction"),
        aspect="auto",
        text_auto=True,
    ...
    feature_fig = px.bar(
        f_importance, x="Feature Name", y="Importance", title="Feature Importance"
    )

    return dbc.Row(
        [
            dbc.Col([draw_Image(feature_fig)], width={"size": 5}),
            dbc.Col([draw_Image(confusion_fig)], width={"size": 3}),
        ]
    )
...
def update_kpi(bp, sex, anaemia):
    ...
    app.logger.info(
        f"Filtered dataframe size: {filtered_df.shape} - saved to file -- {file_name_pd}"
    )
    ...
    return dbc.Row(
        [
            dbc.Col([draw_Text("Observations: " + str(observation_count))], width=3),
            dbc.Col([draw_Text("Death Count: " + str(death_count))], width=3),
            dbc.Col([draw_Text("Survival Count: " + str(no_death_count))], width=3),
        ]
    )
